...
run_deploy()
{
  ...
  CLIENT_ROOT_FOLDER=/Users/julius.reade/Code/PER/datareade/src/packages/datareade_server
...
main()
{
  ...
  do
    if [[ $FILE =~ "src/packages/datareade_server" ]]
    then
      SHOULD_RELEASE='true'
    fi
  done
  ...
  then
    ...
    echo 'datareade_server has no new changes.'
  fi
}
